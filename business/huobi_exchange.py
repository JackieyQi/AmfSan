...
class SignKeyMixin(object):
    ...
    def sign(self, data):
        ...
        dig = hmac.new(secret_key.encode("utf8"), data.encode("utf8"), hashlib.sha256).digest()
...
class HuobiExchangeRequestHandle(SignKeyMixin, BaseExchange):
    ...
    def get_http_data(self, request_data):
        ...
        if self.method == "GET":
            resp = http_get_request("https://"+self.hostname+self.uri+"?", request_data)
        else:
            resp = None
...
class HuobiExchangeAccountHandle(object):
    ...
    def get_current_balance(self):
        result = HuobiExchangeRequestHandle(
            "GET",
            "/v1/account/accounts/{}/balance".format(self.spot_user_id)
        ).get_http_data({})
